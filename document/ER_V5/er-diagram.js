...
const TABLE_COLORS = {
  ...
  recommendation_log:    '#b45309',
  user:                  '#16a34a',
  user_genre_preference: '#15803d',
  user_system_event:     '#166534',
  book:                  '#dc2626',
  book_tag:              '#be185d',
  tag:                   '#db2777',
  genre:                 '#7c3aed',
  publisher:             '#2563eb',
  publisher_promotion:   '#1d4ed8',
  book_store:            '#0891b2',
  book_store_promotion:  '#0e7490',
  language:              '#0d9488',
  device:                '#57534e',
  traffic_source:        '#6b7280',
...
function buildCardEl(tableName) {
  ...
  card.innerHTML = `
    <div class="card-head" style="background:${color}">
      <span>${tbl.label}</span>
      <span class="cbadge">${tbl.type}</span>
    </div>
    <div class="card-body">
      <div class="card-stat" id="stat-${tableName}"></div>
      <div class="card-chips" id="chips-${tableName}"></div>
    </div>`;

  refreshCardBody(tableName, card);
...
function refreshCardBody(tableName, card) {
  const tbl = TABLES[tableName];
  const usedFields = ALL_MODEL_FIELDS[tableName] ? [...ALL_MODEL_FIELDS[tableName]] : [];

  const stat = (card || document.getElementById('card-' + tableName))
    .querySelector('#stat-' + tableName) ||
    document.getElementById('stat-' + tableName);
  if (stat) stat.textContent = `${usedFields.length}/${tbl.fields.length} fields used`;

  const chips = document.getElementById('chips-' + tableName);
  if (!chips) return;
  chips.innerHTML = '';
  usedFields.forEach(f => {
    const ch = document.createElement('span');
    ch.className = 'cchip' + (isFieldActiveInSelectedModel(tableName, f) ? ' active' : '');
    ch.dataset.field = f;
    ch.textContent = f;
...
function openPopup(tableName, cardEl) {
  ...
  // Mark card
  document.querySelectorAll('.er-card').forEach(c => c.classList.remove('popup-active'));
  cardEl.classList.add('popup-active');

  // Build popup HTML
  let html = `<div class="popup-head" style="background:${color}">
    <span>${tbl.label}</span><span class="pbadge">${tbl.type}</span>
  </div>`;

  tbl.fields.forEach(f => {
    const icon = f.pk
      ? '<span class="ppk">🔑</span>'
      : f.fk ? '<span class="pfk">🔗</span>' : '';
    let cls = 'popup-row ';
    if (isFieldActiveInSelectedModel(tableName, f.name))  cls += 'active';
    else if (isFieldUsedInAnyModel(tableName, f.name))    cls += 'used';
    else                                                   cls += 'unused';
  ...
  // Attach field hover/leave for sample-data tooltip
  popup.querySelectorAll('.popup-row').forEach(row => {
  ...
  requestAnimationFrame(() => {
    ...
    if (left + pw > window.innerWidth - 6) left = rect.left - pw - 6;
    let top = rect.top;
...
function closePopup() {
  ...
  document.getElementById('er-popup').classList.remove('visible');
  document.querySelectorAll('.er-card').forEach(c => c.classList.remove('popup-active'));
  hideTooltip();
}

// ========== UPDATE CLASSES AFTER MODEL CHANGE ==========
function updateFieldClasses() {
  // Refresh popup rows
  document.querySelectorAll('#er-popup .popup-row').forEach(row => {
    const t = row.dataset.table, f = row.dataset.field;
    row.classList.remove('active', 'used', 'unused');
    if (isFieldActiveInSelectedModel(t, f))  row.classList.add('active');
    else if (isFieldUsedInAnyModel(t, f))    row.classList.add('used');
...
function onFieldHover(e, tableName, fieldName) {
  ...
  let html = `<div class="tt-head">📊 ${tableName} — <b>${fieldName}</b></div>`;
  html += `<div class="tt-wrap"><table class="tt-tbl"><thead><tr>`;
  data.cols.forEach((c, i) => { html += `<th class="${i === colIdx ? 'hl' : ''}">${c}</th>`; });
  html += `</tr></thead><tbody>`;
  data.rows.forEach(row => {
    html += '<tr>';
    row.forEach((cell, i) => { html += `<td class="${i === colIdx ? 'hl' : ''}" title="${cell}">${cell}</td>`; });
    html += '</tr>';
  });
  html += `</tbody></table></div>`;
  html += `<div class="tt-hint">3 sample rows · "${fieldName}" highlighted</div>`;
...
function positionTooltip(e, tt) {
  ...
  if (x + 460 > window.innerWidth)  x = e.clientX - 460 - m;
...
function selectModel(id) {
  ...
  document.getElementById('active-model-label').innerHTML =
    m ? `Model: <span>#${m.id} ${m.shortName}</span>` : `Model: <span>None selected</span>`;
...
function init() {
  ...
  const allTables = [
    'user_book_event', 'user', 'book', 'recommendation_log',
  ...
  // Sort: tables used in any model first, then unused
  const usedTables   = allTables.filter(n => ALL_MODEL_FIELDS[n] && ALL_MODEL_FIELDS[n].size > 0);
  const unusedTables = allTables.filter(n => !ALL_MODEL_FIELDS[n] || ALL_MODEL_FIELDS[n].size === 0);

  if (usedTables.length) {
    const div = document.createElement('div');
    div.className = 'section-divider';
    div.textContent = '▶ Used in models';
    layout.appendChild(div);
    usedTables.forEach(n => layout.appendChild(buildCardEl(n)));
  }
  if (unusedTables.length) {
    const div = document.createElement('div');
    div.className = 'section-divider';
    div.textContent = '— Not used in models';
    layout.appendChild(div);
    unusedTables.forEach(n => layout.appendChild(buildCardEl(n)));
  }
